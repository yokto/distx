...
int mtx_lock(mtx_t *m)
{
	...
	{
		c = 1;
		__atomic_compare_exchange_n(m, &c, 0, true, __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST);
		if (!c) return 0;
...
int mtx_unlock(mtx_t *m)
{
	...
	else if (__atomic_exchange_n(m, 0, __ATOMIC_SEQ_CST) == 1) return 0;
	...
	for (i = 0; i < 200; i++)
	{
		if (*m)
		{
			...
			uint32_t expected = 2;
			__atomic_compare_exchange_n(m, &expected, 1, true,  __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST);
